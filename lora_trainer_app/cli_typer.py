...
def cmd_generate_data(
    ...
    try:
        ...
        # Apply overrides
        if batch_size:
            config_mgr.dataset_config.batch_size = batch_size
        if max_tokens:
            config_mgr.dataset_config.max_tokens = max_tokens
        if temperature:
            config_mgr.dataset_config.temperature = temperature
        if output_dir:
            config_mgr.dataset_config.output_dir = str(output_dir)
        if seed:
            config_mgr.dataset_config.seed = seed
        
        models = config_mgr.load_models()
        generator = DatasetGenerator(models, config_mgr.dataset_config)
...
def cmd_train_model(
    ...
    try:
        config_mgr = ConfigManager(str(config) if config else None)
        lora_config = config_mgr.lora_config
        
        # Apply overrides
        if dataset:
            lora_config.dataset_path = str(dataset)
        if model_id:
        ...
        if not lora_config.dataset_path:
            console.print("❌ Dataset path is required!", style="bold red")
            raise typer.Exit(1)
...
def cmd_inference(
    ...
    try:
        config_mgr = ConfigManager(str(config) if config else None)
        inf_config = config_mgr.inference_config
        
        # Apply overrides
        if system_prompt:
            inf_config.system_prompt = system_prompt
        if max_tokens:
            inf_config.max_tokens = max_tokens
        if temperature:
            inf_config.temperature = temperature
        if top_p:
            inf_config.top_p = top_p
...
def cmd_export_model(
    ...
    try:
        if gguf_only:
            ...
            console.print(f"🔄 Exporting to GGUF...", style="bold green")
            ...
            console.print(f"🔄 Exporting LoRA model...", style="bold green")
